...
void BoatGrader::initializeChild() { printf("A child has been created.\n"); }

// NEW ADDITION FOR 2014
void BoatGrader::initializeAdult() { printf("An adult has been created.\n"); }
...
void BoatGrader::ChildRideToMolokai() {
    boatAssert(children_on_oahu > 0 && passenger_spaces > 0 && childrenInit > 0,
               "wrong Child arrived on Molokai as a passenger.\n", CONDITION_MISMATCH);
    children_on_oahu--;
    children_on_molokai++;
    passenger_spaces--;
...
void BoatGrader::ChildRideToOahu() {
    boatAssert(children_on_molokai > 0 && passenger_to_oahu > 0 && childrenInit > 0,
               "wrong Child arrived on Oahu as a passenger.\n", CONDITION_MISMATCH);
    children_on_molokai--;
    children_on_oahu++;
    passenger_to_oahu--;
...
int BoatGrader::adultsLeft() { return adults_on_oahu; }

int BoatGrader::childrenLeft() { return children_on_oahu; }

bool BoatGrader::passed() { return children_on_oahu == 0 && adults_on_oahu == 0; }
