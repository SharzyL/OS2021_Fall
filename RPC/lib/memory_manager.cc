...
int MemoryManager::Allocate(int sz) {
    ulock lk(op_mtx);
...
void MemoryManager::Release(int arr_id) {
    ulock lk(op_mtx);
    assert(page_table.count(arr_id) > 0);
    auto &page_table_dir = page_table[arr_id];
    LOG(INFO) << fmt::format("release mem for app {}", arr_id);
