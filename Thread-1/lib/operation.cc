...
void Worker::op_init_emb(int user_idx, const std::vector<int> &item_idx_list) {
    ...
    const Embedding user_emb = get_user_emb(user_idx);
    ...
    for (int item_index : item_idx_list) {
        gradient_futures.emplace_back(std::async(std::launch::async, [item_index, this, &user_emb] {
            const Embedding item_emb = get_item_emb(item_index); // read item
    ...
    update_user_emb(user_idx, total_gradient, 0.01);
...
Embedding Worker::get_user_emb(int idx) {
    shared_lock lock(*user_locks_list[idx]);
    return users[idx];
}

Embedding Worker::get_item_emb(int idx) {
    shared_lock lock(*item_locks_list[idx]);
    return items[idx];
}

void Worker::update_user_emb(int idx, const EmbeddingGradient &grad, double step) {
    unique_lock lock(*user_locks_list[idx]);
    users.update_embedding(idx, grad, step);
}

void Worker::update_item_emb(int idx, const EmbeddingGradient &grad, double step) {
    unique_lock lock(*item_locks_list[idx]);
    items.update_embedding(idx, grad, step);
}
} // namespace proj1
